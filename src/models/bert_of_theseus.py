...
class BERTOfTheseus:
    ...
    def full_training_pipeline(
        ...
        Implements the flowchart (Fig. 6) exactly:
        1. Pre-train Predecessor model
        2. Pre-train Successor model
        3. Train the Mix Model  ←──────────────────────────┐
           └─ Model convergence check (Y/N)                │
              N: loop back (up to max_replacement_rounds)  ┘
              Y: Get the best Successor → Fine-tune
        4. Fine-tune Successor model
        ...
        Args:
            ...
            replacement_epochs: Epochs per Mix Model training round (default: 250)
            fine_tune_epochs: Max epochs for fine-tuning (default: 100)
            learning_rate: Learning rate (default: 0.0001)
            max_replacement_rounds: Max number of "Train Mix Model" rounds before
                                    forcing convergence (guards against infinite loop)
            convergence_threshold: Loss delta below which convergence is declared
        ...
        # ── Step 3: Train the Mix Model — with convergence loop ───────────────
        # Flowchart: Train Mix Model → Model convergence?
        #            N → loop back to Train Mix Model
        #            Y → Get the best Successor
        all_replacement_losses = []
        converged = False

        for round_idx in range(max_replacement_rounds):
            print(f"\n[Mix Model Round {round_idx + 1}/{max_replacement_rounds}]")
            round_losses, converged = self.module_replacement_training(
                train_loader,
                epochs=replacement_epochs,
                learning_rate=learning_rate,
                convergence_threshold=convergence_threshold,
                convergence_window=convergence_window
            )
            all_replacement_losses.extend(round_losses)

            # ── Model convergence Y/N ──────────────────────────────────────
            if converged:  # branch Y: exit the loop
                print(f"[Convergence Y] Proceeding to Get the best Successor.")
                break
            else:           # branch N: iterate again
                print(f"[Convergence N] Looping back to Train Mix Model "
                      f"(round {round_idx + 1} of {max_replacement_rounds}).")

        if not converged:
            print(f"[Convergence N→Y forced] Max rounds ({max_replacement_rounds}) "
                  f"reached. Proceeding with current best Successor.")

        history['replacement_loss'] = all_replacement_losses
