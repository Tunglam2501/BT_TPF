...
class BTPTFTrainer:
    ...
    def train_full_pipeline(
        ...
        # Full training pipeline (includes convergence loop from flowchart Fig.6)
        kd_history = self.bert_of_theseus.full_training_pipeline(
            ...
            learning_rate=self.config.training.learning_rate,
            max_replacement_rounds=self.config.training.max_replacement_rounds,
            convergence_threshold=self.config.training.convergence_threshold,
            convergence_window=self.config.training.convergence_window
        )
    ...
    def evaluate(self, test_loader: DataLoader) -> Dict:
        ...
        Implements the "Test stage" and "Test finish (Y/N)" loop from Fig. 6:
        - Iterates batch by batch through the test set
        - After each batch checks whether all samples have been processed
          N (not finished) → continue to next batch
          Y (finished)     → exit loop, output classification results
        ...
        print(f"\n  [Test stage] Running inference on {total_samples} test samples "
              f"({total_batches} batches)...")

        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(test_loader):
                # ── Test stage: run one batch ──────────────────────────────
                ...
                test_finished = (batch_idx + 1 >= total_batches)  # True = branch Y

                if (batch_idx + 1) % max(1, total_batches // 5) == 0 or test_finished:
                    status = "Y (finished)" if test_finished else "N (continue)"
                    print(f"  [Test finish? {status}] "
                          f"Batch {batch_idx + 1}/{total_batches} "
                          f"— processed {processed}/{total_samples} samples")

                if test_finished:
                    # branch Y: all samples tested → move to Output classification results
                    print(f"  [Test finish Y] All {total_samples} samples tested. "
                          f"Proceeding to Output classification results.")
                    break   # explicit exit matching the flowchart Y branch
